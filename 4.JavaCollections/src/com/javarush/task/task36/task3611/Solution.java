...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
            ...
            if (deep>1) {set.addAll(getAllFriendsAndPotentialFriends(index,deep-1));}
            else set.addAll(addFriendsFirst(index));
            return set;
    }

    public Set<Integer> addFriendsFirst(int index){
        Set<Integer> set = new LinkedHashSet<>();
        for (int q =0; q<humansRelationships.length; q++){
            if (humansRelationships[index][q]==true) set.add(q);
        }
        return set;
    }
